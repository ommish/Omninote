...
demo_hi = Notebook.create!(title: "The hungry bug", user_id: demo.id)
demo_go = Notebook.create!(title: "When is that happening?", user_id: demo.id)
demo_too = Notebook.create!(title: "Reaaaaction", user_id: demo.id)
demo_see = Notebook.create!(title: "Sososososo?", user_id: demo.id)
...
note5 = Note.create!(
  title: "The great kind",
  body_plain: "What a delicious raspberry, yum yum yum",
  body: { ops: [{ retain: 12 }, { delete: 4 }, { insert: 'White', attributes: { color: '#fff' } }]},
  notebook_id: demo_earthworm.id
)
note6 = Note.create!(
  title: "Some helpful things",
  body_plain: "1. do this thing 2. this other thing too 3. apples",
  body: { ops: [ { insert: 'Gandalf ', attributes: { bold: true } }, { insert: 'the ' }, { insert: 'White', attributes: { color: '#fff' } }]},
  notebook_id: demo_earthworm.id
)
note7 = Note.create!(
  title: "Things to drikn",
  body_plain: "1. breakfast 2. lunch 3. dinner",
  body: "{ops: [
  { insert: 'Goooood bye' },
  { insert: '~~~', attributes: { bold: true, color: red }}
]}",
  notebook_id: demo_go.id
)
note8 = Note.create!(
